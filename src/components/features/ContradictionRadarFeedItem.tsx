...
export default function ContradictionRadarFeedItem({
    ...
    const recommendationText = cleanDisplayText(
        ...
        expert?.operational_instructions?.site_diary_draft ||
        ''
    ) || (
    ...
    const legalBasis = cleanDisplayText(
        expert?.contractual_diagnostic?.legal_basis ||
    ...
    const supervisorArgument = cleanDisplayText(
        expert?.contractual_diagnostic?.argument_for_supervisor ||
                    ...
                    >
                        <div className="flex flex-col md:flex-row md:items-center gap-4 p-5 bg-white/[0.02] border border-white/5 rounded-2xl">
                            ...
                            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                                ...
                                <button
                                    ...
                                    className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 bg-blue-500/10 border border-blue-500/20 rounded-xl text-sm font-bold text-blue-400 hover:bg-blue-500/20 transition-all"
                                >
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-white/[0.02] p-6 rounded-2xl border border-white/5">
                                <div className="text-base text-gray-200 leading-8">
                                    <RichText text={cleanDisplayText(c.description || '')} evidence={c.evidence_data} onOpen={onRadarOpenDocument} />
                                </div>
                                {evidenceStatus !== 'VERIFIED' && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="bg-white/[0.02] p-5 rounded-2xl border border-white/5 space-y-3">
                                ...
                                <p className="text-sm text-gray-200 leading-7">
                                    {cleanDisplayText(c.evidence_data?.original_instruction || c.evidence_data?.contract_quote) || 'לא נמצא עדיין ניסוח חוזי ישיר להצגה במסך.'}
                                </p>
                                {c.evidence_data?.clause_reference && (
                            ...
                            <div className="bg-white/[0.02] p-5 rounded-2xl border border-white/5 space-y-3">
                                ...
                                <p className="text-sm text-gray-200 leading-7">
                                    {cleanDisplayText(c.evidence_data?.new_requirement || c.evidence_data?.work_quote) || 'לא נמצא עדיין ניסוח ביצוע ישיר להצגה במסך.'}
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-6 border-t border-white/5">
                            <div className="text-sm text-gray-500">
                                נוצר בתאריך {new Date(c.created_at).toLocaleDateString('he-IL')}
                            </div>
                            <div className="flex items-center gap-3">
                                {c.status !== 'RESOLVED' ? (
                                    <button
